    ...
    if kpi_sugestoes:
        ...
        for i, kpi_sug in enumerate(kpi_sugestoes):
            with kpi_cols[i % min(num_kpis, 4)]:
                ...
                delta_val = str(params.get("delta", "")) # Pega o delta, default para string vazia
                st.metric(label=kpi_sug.get("titulo","KPI"), 
                          value=str(params.get("valor", "N/A")),
                          delta=delta_val if delta_val else None, # Passa None se delta_val for vazio
                          help=params.get("descricao"))
        st.divider()

def render_swot_card(titulo_completo_swot, swot_data, card_key_prefix=""): # card_key_prefix é usado para chaves HTML se necessário
    ...
    for key_swot_category, (header_swot_render, col_target_swot_render) in swot_map.items():
        with col_target_swot_render:
            ...
            for item_swot_render in points_swot_render: 
                # Removido 'key' de st.markdown pois não é um argumento válido
...
st.session_state.pagina_selecionada = st.sidebar.radio("Selecione:", pagina_opcoes_sidebar, 
                                                      index=pagina_opcoes_sidebar.index(st.session_state.pagina_selecionada), 
                                                      key="nav_radio_key_vfinal_swotfix")
st.sidebar.divider(); uploaded_file_sidebar = st.sidebar.file_uploader("Selecione DOCX", type="docx", key="uploader_sidebar_key_vfinal_swotfix")
show_debug_info_sidebar = st.sidebar.checkbox("Mostrar Informações de Depuração", 
                                    value=st.session_state.debug_checkbox_key, 
                                    key="debug_cb_sidebar_key_vfinal_swotfix")
...
if st.session_state.pagina_selecionada == "Dashboard Principal":
            ...
            for item_render_loop in outros_render:
                ...
                with cols_main_dash[idx_main_dash % 2]:
                    ...
                    try:
                        ...
                        st.subheader(item_render_loop["titulo"]) 
    ...
    if not uploaded_file_sidebar: st.warning("Faça upload de um DOCX na barra lateral.")
        ...
        if not swot_sugs_page: st.info("Nenhuma análise SWOT sugerida/selecionada para esta página.")
            ...
            if show_debug_info_sidebar: # Debug específico para dados SWOT
            ...
            for swot_item_render_page in swot_sugs_page:
                render_swot_card(
                    ...
                    card_key_prefix=swot_item_render_page.get("id","swot_page") # Usa o ID da sugestão para o prefixo
                )
...
if uploaded_file_sidebar is None and st.session_state.nome_arquivo_atual is not None:
    ...
    preserved_widget_keys_on_remove = [
        "nav_radio_key_final", "uploader_sidebar_key_final", "debug_cb_sidebar_key_final"
    ] # Adicione outras chaves de widgets principais se tiver
    for key_cfg_sb in st.session_state.get("sugestoes_gemini", []): # Preserva chaves de widgets da sidebar de config
        s_id_preserve = key_cfg_sb.get('id', '')
        if s_id_preserve:
            preserved_widget_keys_on_remove.extend([f"acc_cfg_{s_id_preserve}", f"tit_cfg_{s_id_preserve}"])
            
    for key_cl_remove in keys_to_clear_on_remove:
        if key_cl_remove not in preserved_widget_keys_on_remove:
            # Para evitar erros se a chave já foi deletada por outra lógica
            if key_cl_remove in st.session_state:
                del st.session_state[key_cl_remove]
    
    # Reinicializa os estados principais da aplicação para um novo ciclo
    for k_reinit_remove, dv_reinit_remove in [("sugestoes_gemini",[]),("config_sugestoes",{}),
                                ("conteudo_docx",{"texto":"","tabelas":[]}),
                                ("nome_arquivo_atual",None),("debug_checkbox_key",False), # Usando a chave correta
