...
def extrair_conteudo_docx(uploaded_file):
    try:
        ...
        for i, table_obj in enumerate(document.tables):
            ...
            except Exception: pass # Ignora erro se não conseguir pegar nome da tabela
                    ...
                    for col in df.columns:
                        ...
                        if num_series.notna().sum() / max(1, len(num_series)) > 0.3:
                            ...
                            continue 
                             ...
                             df[col] = original_series # Reverte para original se a conversão numérica não foi boa
                        ...
                        try: # Tenta converter para datetime
                            ...
                            if dt_series.notna().sum() / max(1, len(dt_series)) > 0.5:
                                ...
                                df[col] = original_series.astype(str).fillna('') # Mantém como string se falhou muito
                        except Exception: 
                            df[col] = original_series.astype(str).fillna('')
                    
                    for col in df.columns: # Fallback final para garantir que colunas 'object' sejam string
                        if df[col].dtype == 'object':
                            df[col] = df[col].astype(str).fillna('')
                    tabelas_data.append({"id": f"doc_tabela_{i+1}", "nome": nome_tabela, "dataframe": df})
                except Exception as e_df:
                    st.warning(f"Não foi possível processar DataFrame para tabela {i+1} ({nome_tabela}): {e_df}")
        return "\n\n".join(textos), tabelas_data
    except Exception as e_doc: 
        st.error(f"Erro crítico ao ler DOCX: {e_doc}")
...
def analisar_documento_com_gemini(texto_doc, tabelas_info_list):
    ...
    try:
        ...
        text_limit = 45000 # Reduzido para segurança e evitar timeouts/erros de tamanho
...
def render_swot_card(titulo_completo_swot, swot_data, card_key_prefix=""):
    ...
    for key_swot_render, (header_swot_render, col_target_swot_render) in swot_map.items():
        with col_target_swot_render:
            st.markdown(f"##### {header_swot_render}")
            points_swot_render = swot_data.get(key_swot_render, ["N/A (info. não fornecida)"])
            ...
            for point_idx_swot, item_swot_render in enumerate(points_swot_render): 
                st.markdown(f"<div style='margin-bottom: 5px;'>- {item_swot_render}</div>", unsafe_allow_html=True, 
                            key=f"swot_item_{card_key_prefix}_{titulo_completo_swot.replace(' ','_')}_{key_swot_render}_{point_idx_swot}")
...
def render_plotly_chart(item_config, df_plot_input):
    ...
    if plot_func:
        ...
        if not all(col in df_plot.columns for col in required_cols):
            ...
            return False
        try:
            df_plot_cleaned = df_plot.copy() # Trabalha com cópia para conversões de tipo
            ...
            # Remove NaNs APÓS a tentativa de conversão para numérico nas colunas relevantes
...
# Inicialização de estado concisa
for key_state, default_value_state in [
    ("sugestoes_gemini", []), 
    ("config_sugestoes", {}), 
    ("conteudo_docx", {"texto": "", "tabelas": []}), 
    ("nome_arquivo_atual", None), 
    ("debug_checkbox_key", False), # Renomeado para consistência
    ("pagina_selecionada", "Dashboard Principal")
]:
    st.session_state.setdefault(key_state, default_value_state)

st.sidebar.title("✨ Navegação")
pagina_opcoes_sidebar = ["Dashboard Principal", "Análise SWOT Detalhada"]
st.session_state.pagina_selecionada = st.sidebar.radio(
    ...
    key="nav_radio_key_final" # Chave única
)
st.sidebar.divider()
uploaded_file_sidebar = st.sidebar.file_uploader("Selecione DOCX", type="docx", key="uploader_sidebar_key_final") # Chave única
show_debug_info_sidebar = st.sidebar.checkbox("Mostrar Informações de Depuração", 
                                    value=st.session_state.debug_checkbox_key, 
                                    key="debug_cb_sidebar_key_final") # Chave única
st.session_state.debug_checkbox_key = show_debug_info_sidebar # Sincroniza estado
...
if uploaded_file_sidebar:
        ...
        with st.spinner("Processando novo documento..."):
            ...
            if texto_doc_main or tabelas_doc_main:
                ...
                for i_init_main, s_init_main in enumerate(sugestoes_main): 
                    s_id_init_main = s_init_main.get("id", f"s_init_main_{i_init_main}_{hash(s_init_main.get('titulo',''))}")
                    s_init_main["id"] = s_id_init_main # Garante que a sugestão original tem o ID
    ...
    if st.session_state.sugestoes_gemini:
        ...
        for sug_cfg_sidebar in st.session_state.sugestoes_gemini:
            s_id_cfg_sb = sug_cfg_sidebar['id'] 
            ...
            with st.sidebar.expander(f"{cfg_current_sb['titulo_editado']}",expanded=False):
                ...
                # (Lógica de edição de parâmetros da sidebar pode ser adicionada aqui se necessário)
else: 
    ...
    if uploaded_file_sidebar and st.session_state.sugestoes_gemini:
        ...
        if show_debug_info_sidebar:
             with st.expander("Debug: Elementos para Dashboard Principal (Não-KPI)", expanded=False): 
                st.json({"Outros Elementos (Configurados)": outros_render}, expanded=False)
        
        if outros_render:
            cols_main_dash = st.columns(2); idx_main_dash = 0; count_main_dash = 0
            for item_render_loop in outros_render:
                if item_render_loop.get("tipo_sugerido") == "lista_swot": continue 
                with cols_main_dash[idx_main_dash % 2]:
                    df_plot_loop, rendered_loop = None, False
                    params_loop, tipo_loop, fonte_loop = item_render_loop.get("parametros",{}), item_render_loop.get("tipo_sugerido"), item_render_loop.get("fonte_id")
                    try:
                        if params_loop.get("dados"): 
                            try: df_plot_loop=pd.DataFrame(params_loop["dados"])
                            except Exception as e_dfd_loop: st.warning(f"'{item_render_loop['titulo']}': Erro DF 'dados': {e_dfd_loop}"); continue
                        ...
                        st.subheader(item_render_loop["titulo"]) # Subheader antes de tentar renderizar
                        if tipo_loop=="tabela_dados":
                            ...
                            if str(fonte_loop).startswith("texto_") and params_loop.get("dados"):
                                ...
                                except Exception as e_dftxt_loop: st.warning(f"Erro tabela texto '{item_render_loop['titulo']}': {e_dftxt_loop}")
                            else: id_tbl_loop=params_loop.get("id_tabela_original",fonte_loop); df_tbl_loop=next((t["dataframe"] for t in st.session_state.conteudo_docx["tabelas"] if t["id"]==id_tbl_loop),None)
                            if df_tbl_loop is not None: 
                                try: st.dataframe(df_tbl_loop.astype(str).fillna("-"))
                                except: st.text(df_tbl_loop.to_string(na_rep='-')); 
                                rendered_loop=True
                            else: st.warning(f"Tabela '{item_render_loop['titulo']}' (Fonte: {fonte_loop}) não encontrada.")
                        elif df_plot_loop is not None and tipo_loop in ["grafico_barras","grafico_linha","grafico_dispersao","grafico_pizza"]:
                            if render_plotly_chart(item_render_loop, df_plot_loop): rendered_loop = True
                        elif tipo_loop == 'mapa': st.info(f"Mapa '{item_render_loop['titulo']}' não implementado."); rendered_loop=True
                        
                        if not rendered_loop and tipo_loop not in ["kpi","lista_swot","mapa"]: 
                            st.info(f"'{item_render_loop['titulo']}' ({tipo_loop}) não gerado. Dados/Tipo não suportado.")
                    except Exception as e_render_loop: st.error(f"Erro renderizando '{item_render_loop['titulo']}': {e_render_loop}")
                if rendered_loop: idx_main_dash+=1; count_main_dash+=1
            
            if count_main_dash == 0 and any(c['aceito'] and c['dados_originais'].get('tipo_sugerido') not in ['kpi','lista_swot'] for c in st.session_state.config_sugestoes.values()):
        ...
        elif not kpis_render and not outros_render and uploaded_file_sidebar and st.session_state.sugestoes_gemini: st.info("Nenhum elemento selecionado ou gerado para o dashboard principal.")
...
if uploaded_file_sidebar is None and st.session_state.nome_arquivo_atual is not None:
    ...
    preserved_widget_keys_on_remove = [k_preserve for k_preserve in keys_to_clear_on_remove if k_preserve.endswith(("_key", "_widget_key"))] 
